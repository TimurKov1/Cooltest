...
export function QuestionOne({index, name}) {
    ...
    return (
        <article className={styles.question}>
            <Number index={index}/>
            <textarea className={classnames(styles.question__input, styles.question__textarea)} placeholder="Введите текст..." onChange={changeHandler} rows="3"></textarea>
            ...
            <div className={styles.answers}>
                {
                    answersData.map((item, key) => (
                        <Answer type="radio" name={name} questionIndex={index} index={key} key={key}/>
                    ))
                }
            </div>
        </article>
